...
public static class CommandLineParser
{
    public static bool ParseCommand(Command command, TokenizerResult result)
    {
        ...
        try
        {
            ...
            Logger.LogError($"Error while parsing command arguments. Found {result.Issues.Count} issues:");
    ...
    public static bool ParseCommandOptions(Command command, TokenizerResult result)
    {
        ...
        try
        {
            ...
            if (result.Token.Options.Count < requiredOptionLength)
            {
                ...
                {
                    if (!kvp.Value.IsRequired || optionNames.Contains(kvp.Key)) continue;
            ...
            for (int i = 0; i < result.Token.Options.Count; i++){
                ...
                Logger.LogInfo($"Parsing option token: {token.Name} with {token.Arguments.Count} arguments");
                ...
                Logger.LogInfo($"Option {token.Name} is at index {index} of {result.Token.Options.Count}");
                ...
                if (token.Arguments.Count > def.ArgumentLength)
                {
                    ...
                    {
                        int extraArgs = token.Arguments.Count - def.ArgumentLength;
                        Logger.LogInfo(
                            $"Adding {extraArgs} extra arguments from option {token.Name} to command arguments");
                        
                        result.Token.Arguments.AddRange(token.Arguments.Skip(def.ArgumentLength));
                        token.Arguments.Remove
                    }
                    else
                    {
                        Logger.LogError($"Option {token.Name} has too many arguments and is not last option");
                        return false;
                    }
                }

                if (token.Arguments.Count <= 0) continue;

                Logger.LogInfo(
                    $"Validating argument types for option {token.Name}. Expected type: {def.ValueDefinition.TypeName}");

                if (token.Arguments.All(x => x.TypeName == def.ValueDefinition.TypeName)) continue;
                ...
                foreach (var arg in token.Arguments)
                    Logger.LogError($"Found argument of type {arg.TypeName} with value {arg.Value}");
        ...
        try
        {
            switch (command.Arguments.Count)
            {
                case > 0 when result.Token.Arguments.Count == 0:
                    Logger.LogError($"Command {command.Name} requires arguments but none were provided");
                    return false;
                case 0 when result.Token.Arguments.Count > 0:
                    Logger.LogError(
            ...
            Logger.LogInfo($"Required argument count: {requiredArgLength}, Provided: {result.Token.Arguments.Count}");
            ...
            for (int i = 0; i < result.Token.Arguments.Count; i++)
            {
                ...
                bool hasDefinition = command.Arguments.Count > 1
                ...
                Logger.LogInfo($"Validating argument {i} type. Expected: {def.TypeName}, Found: {token.TypeName}");

                if (def.TypeName != token.TypeName)
                {
                    Logger.LogError($"Argument {i} type mismatch. Expected {def.TypeName}, found {token.TypeName}");
